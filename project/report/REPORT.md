...
## Objectives 
1.	Energy consumption in kWh(minimize) - As our main goal was to achieve most power aware configuration, minimizing energy consumption was one of our primary goals.

2.	Average SLA violation in %(minimize) - Although we wanted our implementation to utilize minimum power we did not expect it to violate SLA more that 0.1%.

3.	Mean time before host shutdown in seconds(maximize) -  This goal was added to make sure that maximum unitizilation of each resource happens.
...
#### Reference pareto fronts generated:

For all the diagrams:
* x-axis : Energy value in kWh
* y-axis : Overall SLA violation in %
* z-axis : Mean time before host shutdown in seconds 

Figure 8 : MOEAD
![](/project/report/MOEAD_reference_pareto.png)

Figure 9 : NSGA
![](/project/report/NSGAII_reference_pareto.png)

Figure 10 : SMPSO
![](/project/report/SMPSO_reference_pareto.png)



## Challenges 
*	Long runtimes for high number of hosts and VMs - As mentioned previously the runs could take upto 2 minutes and 30 seconds in thier worst case. This made very hard to run large model with 1000 of hosts. Eventually, we were suggested to reduce the size in order to get some output from this model.

*	Choice of algorithms  - The outputs you get are largely dependent on the models and the alogrithms you choose. The way one tunes the settings for the algorithm also makes a large difference.

*	Best algorithm depends on the choice of models - Which algorithm will get you the best results cannot be assumed before hand. The alogrithms might or might not be a great fit for your model.

## Future Work 
*	Parallel execution - The model could be speeded up with parallel executions. CloudSim is not completely thread safe. However, while making the model some changes could be made to the core library to allow for parallelization.

*	Comparisons using more algorithms(DE, GA, SPEA2 etc.) - More comaprisons could be made. Curreltly we used only binary domination in our approach. More study could be done using continuous domination.
